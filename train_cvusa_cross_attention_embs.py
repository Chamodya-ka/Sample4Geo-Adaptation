...
import shutil
import sys
from sample4geo.model_crossattention import TimmModel_CA
import torch
import pickle
...
from sample4geo.loss import InfoNCE, MultiSimilarityLoss
...
if __name__ == "__main__":
    ...
    model_path = "{}/{}/{}".format(
        config.model_path, config.model, time.strftime("%H%M%S")
    )
    ...
    model = TimmModel_CA(config.model, pretrained=True, img_size=config.img_size)
    ...
    # Train
    train_dataset = CVUSADatasetTrain(
    ...
    reference_dataset_test = CVUSADatasetEval(
        data_folder=config.data_folder,
        split="test",
        img_type="reference",
    ...
    query_dataset_test = CVUSADatasetEval(
        data_folder=config.data_folder,
        split="test",
        img_type="query",
    ...
    if config.sim_sample:
        ...
        query_dataset_train = CVUSADatasetEval(
            data_folder=config.data_folder,
            split="train",
            img_type="query",
            transforms=ground_transforms_val,
        ...
        reference_dataset_train = CVUSADatasetEval(
            data_folder=config.data_folder,
            split="train",
            img_type="reference",
            transforms=sat_transforms_val,
    ...
    multi_sim_loss_fn = MultiSimilarityLoss(alpha=2.0, beta=50.0, lam=1.0)
    ...
    for epoch in range(1, config.epochs + 1):
        train_dataset.set_epoch(
            epoch
        )  # for debugging, to save images with epoch number
        reference_dataset_test.set_epoch(
            epoch
        )  # for debugging, to save images with epoch number
        query_dataset_test.set_epoch(
            epoch
        )  # for debugging, to save images with epoch number
        query_dataset_train.set_epoch(
            epoch
        )  # for debugging, to save images with epoch number
        reference_dataset_train.set_epoch(
            epoch
        )  # for debugging, to save images with epoch number
        ...
        train_loss = train(
            ...
            scheduler=scheduler,
            scaler=scaler,
            multi_sim_loss_fn=multi_sim_loss_fn,
        )
